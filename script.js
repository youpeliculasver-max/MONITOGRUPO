// ==========================================
// PARTE 1: VARIABLES GLOBALES Y SESIONES
// ==========================================
...
let historial = [];
let graficoEstabilidad = null; // Para Chart.js
...
function iniciarSesion(esUsuario) {
  ...
  if (usuarioExiste) {
    ...
    const confirmar = confirm(`El usuario "${nombre}" no existe. ¿Deseas registrar este perfil para guardar tus mediciones?`);
    if (confirmar) {
...
function abrirDashboard() {
  ...
  // Se ejecutarán cuando peguemos las siguientes partes
...
// ==========================================
// PARTE 2: MOTOR DE MEDICIÓN - LATENCIA Y JITTER
// ==========================================
...
// ==========================================
// PARTE 3: MOTOR DE MEDICIÓN - VELOCIDAD
// ==========================================
async function medirVelocidad() {
  const bytes = 5000000; // 5MB para probar rendimiento real LTE/EPC
...
// ==========================================
// PARTE 4: EVALUACIÓN QoS ESTRICTA
// ==========================================
...
// ==========================================
// PARTE 5: FLUJO PRINCIPAL DE DIAGNÓSTICO
// ==========================================
...
// ==========================================
// PARTE 6: HISTORIAL Y ALMACENAMIENTO
// ==========================================
function guardarEnHistorial(latencia, jitter, velocidad, estado) {
  ...
  if (historial.length > 20) historial.pop(); // Límite de 20 registros
...
// ==========================================
// PARTE 7: CSV Y GRÁFICOS (CHART.JS)
// ==========================================
...
function actualizarGrafico() {
  ...
  // Invertir datos para que el gráfico fluya de izquierda a derecha en el tiempo
  const datosInvertidos = [...historial].reverse();
  const etiquetas = datosInvertidos.map(d => d.fecha.split(',')[1] || "");
  const latencias = datosInvertidos.map(d => parseFloat(d.latencia));
  const velocidades = datosInvertidos.map(d => parseFloat(d.velocidad));
